...
export const generateDispositionPDF = (lease: any) => {
  ...
  doc.setFontSize(12);
  doc.setTextColor(textColor);
  doc.text(lease.tenant?.name || "N/A", leftColX, currentY + 15);
  doc.setFontSize(10);
  doc.setTextColor(lightText);
  ...
  doc.setFontSize(12);
  doc.setTextColor(textColor);
  doc.text(lease.unit?.property?.name || "N/A", rightColX, currentY + 15);
  doc.setFontSize(10);
  doc.setTextColor(lightText);
  ...
  doc.text("Original Deposit:", rightAlign - 120, currentY);
  doc.setTextColor(textColor);
  doc.text(`$${originalDeposit.toFixed(2)}`, rightAlign, currentY, { align: 'right' });
  
  currentY += 20;
  doc.setTextColor(lightText);
  doc.text("Total Deductions:", rightAlign - 120, currentY);
  doc.setTextColor("#EF4444");
  doc.text(`-$${totalDeducted.toFixed(2)}`, rightAlign, currentY, { align: 'right' });
