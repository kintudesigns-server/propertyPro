    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-md bg-white rounded-2xl p-0 border-0 shadow-2xl overflow-hidden flex flex-col max-h-[80vh]">
        <DialogHeader className="p-6 border-b border-[#E2E8F0] bg-[#F8FAFC]">
          <DialogTitle className="text-xl font-bold text-[#0F172A]">New Chat</DialogTitle>
          <p className="text-[#64748B] text-xs font-semibold mt-1">Select a contact to start a conversation</p>
        </DialogHeader>
        ...
        <div className="p-4 border-b border-[#E2E8F0] relative">
          ...
          <input
            ...
            className="pl-10 pr-4 py-2.5 w-full bg-[#F8FAFC] border border-[#E2E8F0] rounded-xl text-sm text-[#0F172A] placeholder-[#94A3B8] focus:outline-none focus:border-[#3B82F6] focus:ring-1 focus:ring-[#3B82F6] transition-all"
          />
        </div>
            ...
            filteredContacts.map((contact) => (
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="h-10 w-10 rounded-full bg-[#EFF6FF] text-[#3B82F6] flex items-center justify-center font-bold text-sm">
                    {contact.name ? contact.name.charAt(0) : "U"}
                  </div>
                  <div>
                    <div className="font-semibold text-sm text-[#0F172A] group-hover:text-[#3B82F6] transition-colors">
                      {contact.name || "User"}
                    </div>
