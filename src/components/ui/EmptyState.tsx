      ...
      {action && (
        action.href ? (
          <Link href={action.href}>
            <Button className="bg-slate-900 hover:bg-slate-800 text-white font-medium rounded-xl h-10 px-5 flex items-center gap-2 text-xs shadow-xs">
              <ActionIcon className="h-4 w-4" /> {action.label}
            </Button>
          </Link>
        ) : (
          <Button onClick={action.onClick} className="bg-slate-900 hover:bg-slate-800 text-white font-medium rounded-xl h-10 px-5 flex items-center gap-2 text-xs shadow-xs">
            <ActionIcon className="h-4 w-4" /> {action.label}
          </Button>
        )
      )}
