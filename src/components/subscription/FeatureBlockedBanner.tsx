...
interface FeatureBlockedBannerProps {
  ...
  /** 
   * When children are provided, they render as the blurred background.
   * When omitted, the component renders ONLY the fixed overlay card —
   * intended for the "overlay" pattern where the page always renders its
   * own real content and this component just overlays the restriction card.
   */
  children?: React.ReactNode;
}

/** 
 * Standalone overlay card — just the fixed centered restriction card with no wrapper.
 * Use this when the page renders its own real content and needs only the overlay.
 */
export function FeatureBlockedOverlay({
  ...
  return (
    <div
      className="fixed inset-0 pl-0 md:pl-64 w-full h-full backdrop-blur-[2.5px] flex items-center justify-center p-4 sm:p-6 z-40 pointer-events-none"
      style={{
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="flex gap-2.5 pt-1">
            <Button
              onClick={() =>
                window.open(
                  `mailto:support@propertypro.com?subject=Access%20Restriction%20Inquiry%20-%20${featureLabel}`
                )
              }
...
/**
 * FeatureBlockedBanner (legacy / wrapper mode).
 * When children are passed, renders them blurred as the background.
 * Still works for any page that explicitly passes children.
 */
export default function FeatureBlockedBanner({
  ...
  return (
    <div className="relative w-full min-h-screen">
      {/* Page content — blurred when wrapped */}
      {children && (
        <div className="w-full pointer-events-none select-none filter blur-[2.5px] opacity-70">
          {children}
        </div>
      )}
