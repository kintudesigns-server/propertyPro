  ...
  return (
    <div className="space-y-8">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        <Card className="bg-white border border-[#E5E5EA] shadow-sm rounded-2xl">
          <CardContent className="p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-sm font-bold text-[#6E6E73]">Total Owners</p>
                <p className="text-2xl font-black text-[#1D1D1F]">{totalOwners}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-white border border-[#E5E5EA] shadow-sm rounded-2xl">
          <CardContent className="p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-sm font-bold text-[#6E6E73]">Active Subscriptions</p>
                <p className="text-2xl font-black text-[#1D1D1F]">{ownersWithPlans}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-white border border-[#E5E5EA] shadow-sm rounded-2xl">
          <CardContent className="p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-sm font-bold text-[#6E6E73]">No Active Plan</p>
                <p className="text-2xl font-black text-[#1D1D1F]">{totalOwners - ownersWithPlans}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-white border border-[#E5E5EA] shadow-sm rounded-2xl">
          <CardContent className="p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-sm font-bold text-[#6E6E73]">Total MRR</p>
                <p className="text-2xl font-black text-[#1D1D1F]">${mrr.toLocaleString(undefined, {minimumFractionDigits: 2})}</p>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-center bg-white p-4 border border-[#E5E5EA] rounded-2xl shadow-sm">
        <div className="relative flex-1 w-full">
          ...
          <Input 
            ...
            className="pl-9 bg-[#F2F2F7] border-transparent focus:bg-white focus:border-blue-500 rounded-xl h-10 w-full"
          />
        </div>
        <div className="flex gap-4 w-full sm:w-auto">
          <Select value={statusFilter} onValueChange={(v) => setStatusFilter(v || "ALL")}>
            <SelectTrigger className="w-full sm:w-[150px] bg-[#F2F2F7] border-transparent rounded-xl h-10 font-medium">
          ...
          <Table>
            <TableHeader className="bg-[#F2F2F7]">
              <TableRow className="border-[#E5E5EA] hover:bg-transparent">
                ...
                <TableHead className="text-[#6E6E73] font-extrabold text-[10px] uppercase tracking-wider">Subscription Tier</TableHead>
                <TableHead className="text-[#6E6E73] font-extrabold text-[10px] uppercase tracking-wider">Status</TableHead>
                <TableHead className="text-[#6E6E73] font-extrabold text-[10px] uppercase tracking-wider">Usage / Limit</TableHead>
                <TableHead className="text-right text-[#6E6E73] font-extrabold text-[10px] uppercase tracking-wider">Properties</TableHead>
            ...
            <TableBody>
              {filteredOwners.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={8} className="h-32 text-center text-[#6E6E73] font-medium">
                ...
                filteredOwners.map((owner, idx) => {
                  ...
                  return (
                  <TableRow key={owner.id} className="border-[#E5E5EA] hover:bg-blue-50/50 transition-colors">
                    <TableCell className="text-[#6E6E73] text-sm font-bold">{idx + 1}</TableCell>
                    <TableCell className="font-bold text-[#1D1D1F]">{owner.name || "Unknown"}</TableCell>
                    ...
                    <TableCell>
                      {owner.pricingTier ? (
                        ...
                        <Badge className="bg-slate-100 text-[#6E6E73] border-0 rounded-lg px-2.5 py-1 font-bold text-xs">
                          No Active Plan
                        </Badge>
                      )}
                    </TableCell>
                    ...
                    <TableCell>
                      {owner.pricingTier ? (
                        <span className={`font-bold ${isOverLimit ? 'text-red-500' : 'text-[#1D1D1F]'}`}>
                          {totalUnits} / {owner.pricingTier.maxUnits} <span className="text-[#6E6E73] font-medium text-xs">Units</span>
                        </span>
                      ) : (
                        <span className="text-[#6E6E73]">{totalUnits} Units</span>
                      )}
                    </TableCell>
                    <TableCell className="text-right">
                      <span className="font-black text-[#1D1D1F] bg-slate-100 px-3 py-1 rounded-md">{owner.ownedProperties.length}</span>
                    </TableCell>
                    <TableCell className="text-right">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="w-56 bg-white rounded-xl shadow-lg border-[#E5E5EA]">
                          <DropdownMenuGroup>
                            ...
                            <DropdownMenuItem 
                              className="flex items-center gap-2 cursor-pointer font-medium text-sm text-[#1D1D1F] focus:bg-[#F2F2F7]" 
                          ...
                          <DropdownMenuGroup>
                            <DropdownMenuItem className="flex items-center gap-2 cursor-pointer font-medium text-sm text-[#1D1D1F] focus:bg-[#F2F2F7]" onClick={() => window.location.href = `mailto:${owner.email}`}>
