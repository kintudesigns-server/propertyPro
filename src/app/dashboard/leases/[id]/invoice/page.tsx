  ...
  return (
    <div className="w-full max-w-5xl mx-auto pt-6 space-y-6 pb-20 px-2 sm:px-0">
      ...
      <div className="flex items-center gap-2 text-sm font-semibold text-[#6E6E73]">
        ...
        <span>/</span>
        <span className="text-[#1D1D1F] truncate max-w-[200px]">Invoice {lease.id.substring(0, 8)}...</span>
      </div>
      ...
      {/* Invoice Document */}
      <Card className="bg-white border-[#E5E5EA] shadow-md rounded-[24px] overflow-hidden">
        {/* Header Block */}
        <div className="p-8 md:p-12 border-b border-[#E5E5EA] flex flex-col md:flex-row justify-between items-start gap-8">
          <div>
          ...
          <div className="text-left md:text-right w-full md:w-auto bg-[#F2F2F7] p-6 rounded-2xl border border-[#E5E5EA]">
            <h2 className="text-xl font-black text-[#1D1D1F] mb-4">INVOICE</h2>
            <div className="grid grid-cols-2 gap-x-8 gap-y-3">
              <p className="text-sm font-bold text-[#6E6E73] text-left md:text-right">Invoice #</p>
              <p className="text-sm font-black text-[#1D1D1F] text-left md:text-right">INV-{lease.id.substring(0,6).toUpperCase()}</p>
              
              <p className="text-sm font-bold text-[#6E6E73] text-left md:text-right">Date Issued</p>
              <p className="text-sm font-black text-[#1D1D1F] text-left md:text-right">{currentDate}</p>
              
              <p className="text-sm font-bold text-[#6E6E73] text-left md:text-right">Date Due</p>
              <p className="text-sm font-black text-[#1D1D1F] text-left md:text-right">{dueDate}</p>
            </div>
        ...
        <div className="p-8 md:p-12 space-y-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            <div>
              ...
              <p className="text-lg font-black text-[#1D1D1F]">{lease.tenant?.name || "Unknown Tenant"}</p>
              <p className="text-sm font-medium text-[#6E6E73] mt-1">{lease.tenant?.email || "No email"}</p>
              <p className="text-sm font-medium text-[#6E6E73]">{lease.tenant?.phone || "No phone"}</p>
            ...
            <div>
              ...
              <p className="text-lg font-black text-[#1D1D1F]">Unit {lease.unit?.name}</p>
              <div className="flex flex-wrap gap-2 mt-2">
                <span className="px-2.5 py-1 bg-[#F1F5F9] text-[#6E6E73] text-xs font-bold rounded-md">{lease.unit?.type || "Apartment"}</span>
                <span className="px-2.5 py-1 bg-[#F1F5F9] text-[#6E6E73] text-xs font-bold rounded-md">{lease.unit?.rooms || 0} Bed</span>
                <span className="px-2.5 py-1 bg-[#F1F5F9] text-[#6E6E73] text-xs font-bold rounded-md">{lease.unit?.bathrooms || 0} Bath</span>
                <span className="px-2.5 py-1 bg-[#F1F5F9] text-[#6E6E73] text-xs font-bold rounded-md">{lease.unit?.sqFootage || 0} SqFt</span>
              </div>
            </div>
          </div>

          {/* Lease Summary */}
          <div className="bg-[#F2F2F7] rounded-2xl p-6 border border-[#E5E5EA]">
            <h3 className="text-sm font-bold text-[#1D1D1F] mb-3">Lease Summary</h3>
            <div className="flex flex-wrap gap-x-8 gap-y-4">
              ...
              <div>
                ...
                <p className="text-sm font-black text-[#1D1D1F]">12 Months</p>
              </div>
            </div>
          </div>
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="text-sm">
                <tr className="border-b border-[#F1F5F9]">
                  <td className="py-4">
                    <p className="font-bold text-[#1D1D1F]">Monthly Rent</p>
                    <p className="text-xs text-[#6E6E73] mt-0.5">Standard monthly residential rent</p>
                  </td>
                  <td className="py-4 text-center font-bold text-[#6E6E73]">1</td>
                  <td className="py-4 text-right font-black text-[#1D1D1F]">${monthlyRent.toLocaleString(undefined, {minimumFractionDigits: 2})}</td>
                </tr>
                <tr className="border-b border-[#F1F5F9]">
                  <td className="py-4">
                    <p className="font-bold text-[#1D1D1F]">Security Deposit</p>
                    <p className="text-xs text-[#6E6E73] mt-0.5">Refundable deposit upon lease termination</p>
                  </td>
                  <td className="py-4 text-center font-bold text-[#6E6E73]">1</td>
                  <td className="py-4 text-right font-black text-[#1D1D1F]">${deposit.toLocaleString(undefined, {minimumFractionDigits: 2})}</td>
                </tr>
                <tr className="border-b border-[#F1F5F9]">
                  <td className="py-4">
                    <p className="font-bold text-[#1D1D1F]">Late Fee</p>
                    <p className="text-xs text-[#6E6E73] mt-0.5">Applied after 5th of the month</p>
                  </td>
                  <td className="py-4 text-center font-bold text-[#6E6E73]">-</td>
                  <td className="py-4 text-right font-black text-[#1D1D1F] text-[#94A3B8] italic">(${lateFee.toLocaleString(undefined, {minimumFractionDigits: 2})})</td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          <div className="flex justify-end pt-6">
            <div className="w-full md:w-1/2 lg:w-1/3 space-y-3">
              <div className="flex justify-between text-sm">
                <span className="font-bold text-[#6E6E73]">Subtotal</span>
                <span className="font-black text-[#1D1D1F]">${subTotal.toLocaleString(undefined, {minimumFractionDigits: 2})}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="font-bold text-[#6E6E73]">Tax (0%)</span>
        ...
        {/* Footer */}
        <div className="bg-[#F2F2F7] p-8 md:p-12 border-t border-[#E5E5EA] text-center md:text-left">
          <p className="text-xs font-bold text-[#94A3B8] uppercase tracking-wider mb-2">Terms & Conditions</p>
          <p className="text-sm text-[#6E6E73] font-medium leading-relaxed max-w-3xl">
        ...
        <DialogContent className="sm:max-w-[500px]">
          ...
          <div className="flex flex-col gap-5 py-4">
            <div className="flex flex-col gap-2">
              <label htmlFor="to" className="text-sm font-semibold text-[#1D1D1F]">To</label>
              <Input
                id="to"
                value={emailTo}
                onChange={(e) => setEmailTo(e.target.value)}
                className="w-full h-11 border-[#E5E5EA] rounded-xl focus-visible:ring-[#007AFF]"
                placeholder="tenant@example.com"
              />
            </div>
            <div className="flex flex-col gap-2">
              <label htmlFor="subject" className="text-sm font-semibold text-[#1D1D1F]">Subject</label>
              <Input
                id="subject"
                value={emailSubject}
            ...
            <div className="flex flex-col gap-2">
              <label htmlFor="message" className="text-sm font-semibold text-[#1D1D1F]">Message</label>
              <textarea
                id="message"
                value={emailMessage}
                onChange={(e) => setEmailMessage(e.target.value)}
                className="w-full flex min-h-[160px] rounded-xl border border-[#E5E5EA] bg-transparent px-3 py-3 text-sm shadow-sm placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#007AFF] disabled:cursor-not-allowed disabled:opacity-50"
              />
            </div>
          </div>
          <DialogFooter>
            <Button type="button" variant="outline" className="rounded-xl" onClick={() => setIsEmailModalOpen(false)}>Cancel</Button>
            <Button onClick={handleSendEmail} disabled={isSending} className="rounded-xl bg-[#007AFF] hover:bg-[#0062CC] text-white">
              {isSending ? (
                <>Sending...</>
              ) : (
