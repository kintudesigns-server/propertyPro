    ...
    <div className="w-full max-w-7xl mx-auto pt-4 space-y-6 pb-20 px-2 sm:px-6 font-sans">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-5 sm:p-6 rounded-3xl border border-slate-200 shadow-xs">
        <div className="flex flex-col gap-1.5">
          <Link href={`/dashboard/properties/${id}`} className="text-xs font-medium text-slate-500 hover:text-slate-900 flex items-center gap-1.5 transition-colors w-fit">
            <ArrowLeft className="h-3.5 w-3.5" /> Back to Property
          </Link>
          <div className="flex items-center gap-3 flex-wrap">
            <h1 className="text-3xl font-semibold text-slate-900 tracking-tight">{unit.name}</h1>
            <div className="flex items-center gap-1.5">
              {unit.status === "VACANT" ? (
                ...
                <span className="px-2.5 py-0.5 rounded-md text-[10px] font-semibold uppercase tracking-wider bg-rose-50 text-rose-800 border border-rose-200 shadow-2xs">{unit.status}</span>
              )}
          ...
          <div className="flex items-center gap-1 text-xs font-semibold text-slate-500">
            <Building className="h-3.5 w-3.5 text-slate-400" /> Belongs to <Link href={`/dashboard/properties/${id}`} className="text-slate-900 font-medium hover:underline">{property.name}</Link>
          </div>
        </div>
      ...
      <div className="min-h-[400px]">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <Card className="col-span-1 lg:col-span-2 bg-white border border-slate-200 rounded-3xl shadow-xs">
                <div className="p-5 sm:p-6 border-b border-slate-100 bg-slate-50/50">
                  <h2 className="font-semibold text-slate-900 text-base tracking-tight">Unit Specifications</h2>
                </div>
                <CardContent className="p-6">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div className="space-y-3">
                      <div className="grid grid-cols-2 gap-4 border-b border-slate-100 pb-3">
                        <div className="text-xs font-normal text-[#6E6E73]">Unit Name/Number</div>
                        <div className="font-semibold text-[#1D1D1F] text-xs">{unit.name}</div>
                      </div>
                      <div className="grid grid-cols-2 gap-4 border-b border-slate-100 pb-3">
                        <div className="text-xs font-normal text-[#6E6E73]">Unit Type</div>
                        <div className="font-semibold text-[#1D1D1F] text-xs">{unit.type || "Apartment"}</div>
                      </div>
                      <div className="grid grid-cols-2 gap-4 border-b border-slate-100 pb-3">
                        <div className="text-xs font-normal text-[#6E6E73]">Floor Level</div>
                        <div className="font-semibold text-[#1D1D1F] text-xs">{unit.floor || "1"}</div>
                      </div>
                    </div>
                    <div className="space-y-3">
                      <div className="grid grid-cols-2 gap-4 border-b border-slate-100 pb-3">
                        <div className="text-xs font-normal text-[#6E6E73]">Deposit Required</div>
                        <div className="font-semibold text-[#1D1D1F] text-xs">${Number(unit.depositAmt).toFixed(2)}</div>
                      </div>
                      <div className="grid grid-cols-2 gap-4 border-b border-slate-100 pb-3">
                        <div className="text-xs font-normal text-[#6E6E73]">Current Status</div>
                        <div className="font-semibold text-[#1D1D1F] text-xs">{unit.status}</div>
                      </div>
                    </div>
                  </div>
                </CardContent>
                ...
                <Card className="bg-white border border-slate-200 rounded-3xl shadow-xs">
                  ...
                  <CardContent className="p-5">
                    ...
                    <Link href={`/dashboard/properties/${id}`}>
                      <Button className="w-full font-medium text-xs h-9 bg-white text-slate-900 border border-slate-200 hover:bg-slate-50 rounded-xl shadow-2xs cursor-pointer">View Property</Button>
                    </Link>
                  </CardContent>
                </Card>
              </div>
            ...
            {activeLease ? (
              <Card className="bg-white border border-slate-200 rounded-3xl shadow-xs">
                <div className="p-5 sm:p-6 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
                  ...
                  <span className="px-2.5 py-0.5 rounded-md text-[10px] font-semibold uppercase tracking-wider bg-emerald-50 text-emerald-800 border border-emerald-200 shadow-2xs">Active</span>
                </div>
                <CardContent className="p-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-4">
                      <h3 className="text-[10px] font-semibold text-[#6E6E73] uppercase tracking-wider mb-2">Tenant Details</h3>
                      <div className="flex items-center gap-3 border-b border-slate-100 pb-4">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-[#6E6E73] font-normal">{activeLease.tenant?.email}</p>
                        </div>
                      </div>
                    </div>
                    <div className="space-y-4">
                      <h3 className="text-[10px] font-semibold text-[#6E6E73] uppercase tracking-wider mb-2">Lease Terms</h3>
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <p className="text-[10px] font-semibold text-[#6E6E73] uppercase tracking-wider">Start Date</p>
                          <p className="font-black text-slate-900 text-xs mt-0.5">{new Date(activeLease.startDate).toLocaleDateString()}</p>
                        </div>
                        <div>
                          <p className="text-[10px] font-semibold text-[#6E6E73] uppercase tracking-wider">End Date</p>
                          <p className="font-black text-slate-900 text-xs mt-0.5">{new Date(activeLease.endDate).toLocaleDateString()}</p>
                        </div>
                        <div>
                          <p className="text-[10px] font-semibold text-[#6E6E73] uppercase tracking-wider">Rent Amount</p>
              ...
              <div className="py-16 text-center border-2 border-dashed border-slate-200 rounded-3xl bg-slate-50">
                ...
                <p className="text-xs text-[#6E6E73] font-normal mt-1 max-w-sm mx-auto">This unit is currently vacant. There are no active leases or tenants assigned to it.</p>
                <Button className="mt-4 bg-slate-900 hover:bg-slate-800 text-white font-medium text-xs rounded-xl h-9 px-4 shadow-xs cursor-pointer">
                  Create Lease
                </Button>
              </div>
            )}
        ...
        {activeTab === "images" && (
          <div className="space-y-6">
            <div className="flex justify-between items-center">
              <div>
                <h2 className="text-xl font-semibold text-slate-900 tracking-tight">Unit Images</h2>
                <p className="text-xs font-semibold text-slate-500 mt-0.5">Showing {unit.images?.length || 0} images for this unit.</p>
              </div>
              <Button className="bg-slate-900 hover:bg-slate-800 text-white font-medium text-xs shadow-xs rounded-xl h-9 px-4 cursor-pointer">
                Add Image
              </Button>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {unit.images?.length > 0 ? (
                ...
                <div className="col-span-full py-16 text-center border-2 border-dashed border-slate-200 rounded-3xl bg-slate-50">
                  ...
                  <p className="text-xs text-[#6E6E73] font-normal mt-0.5">Upload interior photos for this specific unit.</p>
                </div>
              )}
            </div>
          </div>
        ...
        {activeTab === "amenities" && (
          <div className="space-y-6">
            <div className="flex justify-between items-center">
              <div>
                <h2 className="text-xl font-semibold text-slate-900 tracking-tight">Unit Amenities</h2>
            ...
            {unit.amenities && unit.amenities.length > 0 ? (
              ...
              <div className="py-16 text-center border-2 border-dashed border-slate-200 rounded-3xl bg-slate-50">
                ...
                <p className="text-xs text-[#6E6E73] font-normal mt-0.5">No amenities have been specified for this unit yet.</p>
              </div>
            )}
          </div>
        )}
  ...
  return (
    <div className="bg-white border border-slate-200 shadow-xs rounded-3xl p-5 flex flex-col justify-between h-full">
      <div className="flex justify-between items-start mb-3">
        <p className="text-xs font-normal text-[#6E6E73] pr-4">{title}</p>
        <div className="p-2.5 bg-slate-100 text-slate-800 border border-slate-200/80 rounded-2xl shrink-0">
          <Icon className="h-4 w-4" />
        </div>
      </div>
      <div>
        <p className="font-semibold text-[#1D1D1F] text-2xl leading-tight truncate">{value}</p>
        {subtext && <p className="text-xs text-[#6E6E73] font-normal mt-0.5">{subtext}</p>}
      </div>
    </div>
  );
}
