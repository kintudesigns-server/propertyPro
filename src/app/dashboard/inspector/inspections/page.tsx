  ...
  if (loading || status === "loading") {
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] gap-4">
        ...
        <p className="text-[#8E8E93] font-extrabold text-sm tracking-wider uppercase">Loading inspections...</p>
      </div>
    );
  }
      ...
      <Tabs defaultValue="scheduled" className="w-full">
          ...
          {scheduledWalkthroughs.length === 0 ? (
            <Card className="border-dashed border-2 border-slate-200 shadow-none py-12 rounded-2xl">
              <CardContent className="text-center space-y-2">
                <Calendar className="h-10 w-10 text-[#8E8E93] mx-auto" />
                <h3 className="font-bold text-[#1D1D1F] text-lg">No Scheduled Walkthroughs</h3>
                <p className="text-[#6E6E73] text-sm font-semibold max-w-sm mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {scheduledWalkthroughs.map((w: any, idx: number) => (
                <Card key={idx} className="border-[#E5E5EA] shadow-sm rounded-2xl hover:border-indigo-100 transition-colors">
                  <CardHeader className="pb-3">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-xs text-[#8E8E93] font-bold">{w.statusLabel}</span>
                    </div>
                    <CardTitle className="text-lg font-black text-[#1D1D1F]">
                      {w.unit?.property?.name}
                    </CardTitle>
                    <CardDescription className="flex items-center gap-1 font-semibold text-[#6E6E73]">
                  ...
                  <CardContent className="space-y-4">
                    <div className="grid grid-cols-2 gap-4 bg-slate-50 p-3 rounded-xl border border-slate-100 text-xs">
                      <div>
                        <p className="text-[#8E8E93] font-bold uppercase tracking-wider mb-0.5">Tenant</p>
                        <p className="font-extrabold text-[#1D1D1F]">{w.tenant?.name || "N/A"}</p>
                      </div>
                      <div>
                        <p className="text-[#8E8E93] font-bold uppercase tracking-wider mb-0.5">Target Date</p>
                        <p className="font-extrabold text-[#1D1D1F]">{w.date ? new Date(w.date).toLocaleDateString() : "N/A"}</p>
                      </div>
                    </div>
        ...
        <TabsContent value="completed" className="space-y-4 outline-none">
          {completedWalkthroughs.length === 0 ? (
            <Card className="border-dashed border-2 border-slate-200 shadow-none py-12 rounded-2xl">
              <CardContent className="text-center space-y-2">
                <CheckCircle2 className="h-10 w-10 text-[#8E8E93] mx-auto" />
                <h3 className="font-bold text-[#1D1D1F] text-lg">No Completed Walkthroughs</h3>
                <p className="text-[#6E6E73] text-sm font-semibold max-w-sm mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {completedWalkthroughs.map((w: any, idx: number) => (
                <Card key={idx} className="border-[#E5E5EA] shadow-sm rounded-2xl bg-slate-50/30">
                  ...
                  <CardContent className="p-5 space-y-4">
                    <div className="grid grid-cols-2 gap-4 text-xs font-semibold text-[#6E6E73]">
                      <div>
                        <span className="text-[#8E8E93] font-bold uppercase tracking-wider block mb-0.5">Tenant</span>
                        <span className="font-extrabold text-[#1D1D1F]">{w.tenant?.name || "N/A"}</span>
                      </div>
                      <div>
                        <span className="text-[#8E8E93] font-bold uppercase tracking-wider block mb-0.5">Deductions Logged</span>
                    ...
                    {w.notes && (
                      <div className="bg-white p-3 rounded-xl border border-slate-200 text-xs">
                        <strong className="block text-[#8E8E93] font-bold uppercase tracking-wider mb-1">Notes Summary</strong>
                        <p className="text-slate-700 font-medium line-clamp-2">{w.notes}</p>
                      </div>
                    )}

                    {w.signedAt && (
                      <div className="text-[11px] text-[#8E8E93] font-medium pb-2">
                    ...
                    <Button
                      ...
                      className="w-full bg-white border border-slate-200 text-slate-700 hover:bg-[#F5F5F7] font-bold rounded-xl h-10 text-xs shadow-none mt-2 flex items-center justify-center gap-2"
                    >
          ...
          <div className="space-y-4 my-4 max-h-[60vh] overflow-y-auto pr-1">
            ...
            {selectedLeaseForDetails?.notes && (
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                <p className="text-xs font-black text-[#8E8E93] uppercase tracking-wider mb-1">Inspector Notes</p>
            ...
            <div className="space-y-3">
              <p className="text-xs font-black text-[#8E8E93] uppercase tracking-wider">Flagged Issues ({selectedLeaseForDetails?.deductionsCount || 0})</p>
              {(() => {
