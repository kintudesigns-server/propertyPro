...
const PRIORITY_CFG: Record<string, { label: string; dot: string; text: string; bg: string; border: string }> = {
  ...
  LOW:       { label: "Low",       dot: "bg-slate-400",  text: "text-slate-500",  bg: "bg-slate-50",  border: "border-slate-200" },
};
  ...
  if (loading || status === "loading") {
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] gap-4">
        ...
        <p className="text-slate-400 font-semibold text-sm">Loading history logs...</p>
      </div>
    );
  }
    ...
    <div className="max-w-[1400px] mx-auto px-4 sm:px-6 pt-6 pb-20 space-y-5">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-0.5">A complete record of all resolved and closed repair requests.</p>
        </div>
        <div className="flex items-center gap-2 text-xs font-bold text-slate-500 bg-white border border-slate-200 rounded-xl px-4 py-2">
        ...
        <div className="flex items-center gap-1 bg-white border border-slate-200 rounded-xl p-1 shadow-xs">
          {["ALL", "RESOLVED", "CLOSED"].map(s => (
            <button key={s} onClick={() => setStatusFilter(s)}
              className={`px-3 py-1.5 rounded-lg text-[11px] font-bold transition-all ${statusFilter === s ? "bg-slate-900 text-white shadow-xs" : "text-slate-500 hover:bg-slate-100"}`}>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl shadow-xs overflow-hidden">
              ...
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200">
                  ...
                  <th className="text-left px-5 py-3 w-[150px]">
                    <button onClick={() => toggleSort("date")} className="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest hover:text-slate-600 transition-colors flex items-center">
                ...
                {filtered.map(t => {
                  ...
                  return (
                    <React.Fragment key={t.id}>
                      <tr className="hover:bg-slate-50/70 transition-colors cursor-pointer" onClick={() => setExpanded(isExpanded ? null : t.id)}>
                        {/* Work order title */}
                        ...
                        <td className="px-5 py-4">
                          <div className="flex items-start gap-1.5">
                            ...
                            <div>
                              ...
                              <p className="text-[11px] text-slate-400 font-medium">Unit {t.unit?.name}</p>
                            </div>
                          </div>
                        </td>
                          ...
                          <div className="flex items-center justify-end gap-2">
                            {isResolved && (
                              <Button
                                ...
                                className="h-8 text-[11px] font-bold bg-slate-900 hover:bg-slate-800 text-white rounded-lg px-3 shadow-xs"
                              >
                                {updatingId === t.id ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : "Archive"}
                              </Button>
                            )}
                            <DropdownMenu>
                              <DropdownMenuTrigger className="h-8 w-8 rounded-lg border border-slate-200 bg-white hover:bg-slate-50 flex items-center justify-center transition-colors shadow-xs">
                        ...
                        <tr className="bg-slate-50/80 border-l-2 border-l-emerald-400">
                          <td colSpan={6} className="px-6 py-4">
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
                              <div>
                                <p className="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest mb-1.5">Resolution description</p>
                                <p className="text-slate-700 text-xs leading-relaxed">{t.inspectorNotes || "No resolution comments logged."}</p>
                              </div>
                              <div>
                                <p className="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest mb-1.5">Tenant Feedback</p>
                                {t.tenantRating || t.tenantFeedback ? (
                                  <div className="space-y-1">
                                    <p className="text-xs font-bold text-amber-600">Rating: {t.tenantRating || "—"} / 5 ★</p>
                                    {t.tenantFeedback && <p className="text-xs text-slate-600 italic">"{t.tenantFeedback}"</p>}
                                  </div>
                                ) : (
                              ...
                              <div>
                                <p className="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest mb-1.5">Final Cost Billing</p>
                                <div className="space-y-1">
                                  <div className="flex justify-between text-xs text-slate-600"><span>Labor:</span><span className="font-bold">${Number(t.finalLabor || 0).toFixed(2)}</span></div>
                                  <div className="flex justify-between text-xs text-slate-600"><span>Materials:</span><span className="font-bold">${Number(t.finalMaterials || 0).toFixed(2)}</span></div>
          ...
          <div className="px-5 py-3 border-t border-slate-100 bg-slate-50/60 flex items-center justify-between">
            <p className="text-xs text-slate-400 font-semibold">
              Showing <strong className="text-slate-600">{filtered.length}</strong> of <strong className="text-slate-600">{completedTasks.length}</strong> completed requests
            </p>
